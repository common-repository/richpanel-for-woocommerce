...
<div class="rp-data-sync--subtext">
  ...
  <option value="resume">Resume Last Sync</option>
...
<script>
const $ = jQuery
let apiKey = '<?php echo esc_attr($this->rp_api_key); ?>';
let apiSecret = '<?php echo esc_attr($this->rp_api_secret); ?>';
...
$("#orderSyncButton").text(`Sync Orders`);
$("#subscriptionSyncButton").text(`Sync Subscription`);
enableSyncButtons()
}
...
$('#orderSyncButton').off().on('click', () => location.href = "<?php echo esc_url(get_admin_url(null, 'admin.php?page=richpanel-admin')); ?>&import=1&importdates="+importdates)
...
var idates = <?php echo !empty($_GET['importdates']) ? ( 'resume' === sanitize_text_field($_GET['importdates']) ? "'" . esc_js(json_encode(sanitize_text_field($_GET['importdates']))) . "'" : esc_js(json_encode(intval(sanitize_text_field($_GET['importdates'])))) ) : 'undefined'; ?>;
idates = (idates== "undefined") ? 6 : idates;
localStorage.setItem("lastSyncPeriod", idates);
setOrderSyncProgress(progress_percents)
$.post("<?php echo esc_html_e(admin_url('admin-ajax.php')); ?>", {'action': 'richpanel_chunk_sync','idates':idates, 'chunk_page': chunk_page}, function(response) {

const new_chunk_page = parseInt(chunk_page) + 1;
if(new_chunk_page <= chunk_pages){
localStorage.setItem("sync_chunk_page", new_chunk_page);
const v1 = localStorage.getItem("sync_chunk_page");
setTimeout(function(){
...
if ( 'true' === "<?php echo ( isset( $_GET['importdates'] ) && 'resume' == $_GET['importdates'] ) ? 'true' : 'false'; ?>" )
{
// Retrieve the value of chunk_page from local storage
var chunk_page = localStorage.getItem("sync_chunk_page") || 0;
sync_chunk(chunk_page);
}else{
sync_chunk(0);
}
