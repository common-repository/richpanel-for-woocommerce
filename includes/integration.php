...
if ( ! class_exists( 'Richpanel_Woo_Analytics_Integration' ) ) :
	...
	class Richpanel_Woo_Analytics_Integration extends WC_Integration {
		...
		private $integration_version = '2.2.2';
		...
		public  $tracking_endpoint_domain = 'cdn.richpanel.com';
		...
		public function __construct() {
			...
			$this->rp_api_key = $this->get_option('rp_api_key', false); //check here
			$this->rp_api_secret = $this->get_option('rp_api_secret', false); //check here
			...
			// previous version compatibility - fetch api_key and api_secret
			if (empty($this->rp_api_key) && empty($this->rp_api_secret)) {
				$this->rp_api_key = $this->get_previous_version_api_key();
				$this->rp_api_secret = $this->get_previous_version_api_secret();	
			}
			// previous version compatibility - fetch token from Wordpress settings
			if (empty($this->rp_api_key)) {
				$this->rp_api_key = $this->get_previous_version_settings_key();
			}
			if (empty($this->rp_api_secret)) {
				$this->rp_api_secret = false;
			}
		...
		public function prepare_order_import() {
			global $wpdb;
			if (isset($_REQUEST['idates'])) {
				if ('resume' == $_REQUEST['idates']) {
					$dateVal = '<script>document.write(localStorage.getItem("lastSyncPeriod"));</script>';
					$dateMinus12 = gmdate('Y-m-d', strtotime('- ' . $dateVal . ' Months'));
				} else {
					$dateVal = intval($_REQUEST['idates']); // Sanitize the input to an integer value
					$dateMinus12 = gmdate('Y-m-d', strtotime('-' . $dateVal . ' Months'));
				}
			}
			$dateMinus12 = isset($dateMinus12) ? $dateMinus12 : gmdate('Y-m-d', strtotime('-6 Months')); 
			//HPOS compatibility changes
			$args = array(
				'type' => 'shop_order',
				'date_created' => '>=' . $dateMinus12,
				'return' => 'ids',
				'limit' => -1 // Retrieve all orders
			);
			$this->orders_total = count(wc_get_orders($args));
			$args['type'] = 'shop_subscription';
			$this->subscription_total = count(wc_get_orders($args));
		...
		public function validate_endpoint_request_id( $req_id, $endpoint) {
			...
			$end_point_params = array('req_id' => $req_id, 'endpoint' => $endpoint, 'appClientId' => $this->rp_api_key);
		...
		public function recent_orders_sync( $days_sync) {
			...
			// prepare query
			$date_after = gmdate('Y-m-d', strtotime("-{$days_sync} days"));
			...
			try {
				...
				$signature = md5($based_call . $this->rp_api_secret);
		...
		public function check_for_keys() {
			if (is_admin()) {
				// check_admin_referer();
				if (( empty($this->rp_api_key) || empty($this->rp_api_secret) ) && empty($_POST['save'])) {
					add_action('admin_notices', array($this, 'admin_keys_notice'));
				}
				if (!empty($_POST['save']) && !empty($this->rp_api_key) && !empty($_POST['woocommerce_richpanel-woo-analytics_api_key'])) {
		...
		public function ensure_hooks() {
			...
			add_action('wp_ajax_richpanel_clear_events', array($this, 'check_for_richpanel_clear'));
			add_action('wp_ajax_nopriv_richpanel_clear_events', array($this, 'check_for_richpanel_clear'));
		...
		public function subscription_status_update_event( $subscription, $new_status, $old_status) {
			...
			try {
				...
				$purchase_params = $this->prepare_order_params($order, array('old_status' => $old_status, 'status' => $new_status));
				...
				foreach ($order_items as $item_id => $item_data) {
					...
					$product_hash = $this->prepare_product_hash($product_, $item_data['variation_id']);

					$product_hash['contains'] = array(
						'lineItemId' => $item_id
					);

					$product_hash['quantity'] = wc_get_order_item_meta($item_id, '_qty', true);
					$product_hash['totalPrice'] = wc_get_order_item_meta($item_id, '_line_total', true);
		...
		public function richpanel_admin_page() {
			...
			if (isset($_POST['woocommerce_richpanel-woo-analytics_api_key']) && isset($_POST['woocommerce_richpanel-woo-analytics_api_secret'])) {
				$this->rp_api_key = sanitize_text_field($_POST['woocommerce_richpanel-woo-analytics_api_key']);
				$this->rp_api_secret = sanitize_text_field($_POST['woocommerce_richpanel-woo-analytics_api_secret']);
				if (method_exists($this, 'update_option')) {
					// New Version
					$this->update_option('rp_api_key', $this->rp_api_key);
					$this->update_option('rp_api_secret', $this->rp_api_secret);
				} else {
					// Old Version
					update_option('api_key', $this->rp_api_key);
					update_option('api_secret', $this->rp_api_secret);
				}
			}
		...
		public function sync_orders_chunk( $specific_order_ids = false) {
			...
			//$dateMinus12 = gmdate('Y-m-d', strtotime('-240 Months')); // Last day 12 months ago
			if ( isset( $_REQUEST['idates'] ) && ( 'resume' == $_REQUEST['idates'] ) ) {
				$dateVal = '<script>document.write(localStorage.getItem("lastSyncPeriod"));</script>';
				$dateMinus12 = gmdate('Y-m-d', strtotime('- ' . $dateVal . ' Months'));
			} else {
				if (isset($_REQUEST['idates'])) {
				$dateVal = intval($_REQUEST['idates']); // Sanitize the input to an integer value
				$dateMinus12 = gmdate('Y-m-d', strtotime('-' . $dateVal . ' Months')); // Last day 12 months ago
				}
			}
			...
			if (!$specific_order_ids) {
				...
				if (isset($_REQUEST['chunk_page'])) { 
					$sync_chunk_page = '<script>document.write(localStorage.getItem("sync_chunk_page"));</script>' || 2;
					$chunk_page = ( isset( $_REQUEST['chunk_page'] ) && 'resume' == $_REQUEST['idates'] ) ? (int) $sync_chunk_page : (int) $_REQUEST['chunk_page'];
					...
					$args = array(
						...
						'orderby' => 'date',
						'order' => 'ASC',
						'limit' => $this->orders_per_import_chunk,
						'offset' => $chunk_offset
					);
					$order_ids = wc_get_orders($args);
				}
				...
				foreach ($order_ids as $order_id) {
					try {
						...
						if (!empty($order_id) && !empty($order)) {
							...
							$did = null;
							$sid = null;
							$uid = isset($purchase_params['user_identity']) ? $purchase_params['user_identity']: $identity_data['email']; //check the syntax
							$user_id = $order->get_user_id();
							$call_params['userProperties'] = $this->getUserInfoBatch($purchase_params, $user_id);
		...
		public function sync_subscriptions_chunk( $specific_order_ids = false) {
			// Ensure the orders_total and subscription_total properties are set
			$this->prepare_order_import();
			global $wpdb;
		...
		public function encryptData( $data) {
			...
			$rp_api_secret = $this->rp_api_secret;

			$method = 'AES-256-CBC';
			$key = hash('sha256', $rp_api_secret);
		...
		public function fillUserDataWithOrderData( $properties) {
			...
			$email = isset($properties['email']) ? $properties['email'] : (isset($properties['billing_email']) ? $properties['billing_email'] : $properties['shipping_email']);
			if (!$email) {
				return null; // If no email is found at all, return null.
			}
		...
		private function build_call( $did, $sid, $ident, $event, $properties, $identity_data = false, $time = false, $call_params = false) {
			...
			if ($call_params) {
				if (isset($call_params['ip']) && $call_params['ip']) {
		...
		public function prepare_variation_data( $variation_id, $variation = false) {
			...
			if ($variation_obj) {
				if (!empty($variation_data['name'])) {
					...
					$variation_data['name'] = $variation_obj->get_sku();
				}
		...
		public function get_order_status( $order_object) {
			...
			public function prepare_order_params( $order, $order_merge_params = array()) {
				...
				// prepare basic order data
					...
					'email' 		=> $this->object_property($order, 'order', 'email'),
					'first_name' 	=>  $this->object_property($order, 'order', 'first_name'),
					'last_name' 	=>  $this->object_property($order, 'order', 'last_name'),
					);
				...
				try {
					$purchase_params['totalRefundedAmount'] = method_exists($order, 'get_total_refunded') ? $order->get_total_refunded() : 0;

					// $order_data = $order->data;

					// To get specific
					$trackingFields = get_post_meta( $purchase_params['order_id'], '_wc_shipment_tracking_items', true );

					if (empty($trackingFields)) {
						$trackingFields = $order->get_meta('_wc_shipment_tracking_items');
					}
					...
					if (!empty($trackingFields)) {
						...
						if (!empty($trackingFields)) {
							...
							if (!empty($trackingFields['order_date'])) {
								$purchase_params['shippingDate'] = (int) strtotime($trackingFields['order_date']) * 1000;
							}
						} 
					}
				} catch (Exception $e) {
				...
				// tracking details from items, if any
				try {
					$fulfilmentIdsTemp = array();
					$tempIndex = 0;
					$order_items = $order->get_items();
					foreach ($order_items as $item_id => $item_data) {
						$fulfillment_status = $item_data->get_meta('Fulfillment Status');
						$provider = $item_data->get_meta('Provider');
						$tracking_number = $item_data->get_meta('Tracking Number');
						$tracking_link = $item_data->get_meta('Tracking Link');
						$date_shipped = $item_data->get_meta('Date Shipped');
						if (empty($tracking_number)) {
							continue;
						}
						$product_ = $this->resolve_product($item_data['product_id']);
						$itemObject = $this->prepare_product_hash($product_, $item_data['variation_id']);
						$itemObject['quantity'] = wc_get_order_item_meta($item_id, '_qty', true);
						unset($itemObject['image_url']);
						unset($itemObject['categories']);
						$fid = $purchase_params['order_db_key'] . $tracking_number;
						if (!array_key_exists($fid, $fulfilmentIdsTemp)) {
							$fulfilmentIdsTemp[$fid] = $tempIndex;
							$tempIndex = ++$tempIndex;
							$fObject = array(
								'id' => $fid,
								'items' => [$itemObject],
								...
								'tracking' => [array(
									...
									'trackingCompany' => $provider,
									'trackingUrl' => $tracking_link,
									'shippingDate' => strtotime($date_shipped) * 1000
								)]
							);
							$purchase_params['fulfillment'][] = $fObject;
						} else {
							$index = $fulfilmentIdsTemp[$fid];
							$purchase_params['fulfillment'][$index]['items'][] = $itemObject;
							$purchase_params['fulfillment'][$index]['quantity'] = $purchase_params['fulfillment'][$index]['quantity'] + (int) $itemObject['quantity'];
						}
					}
				} catch (Exception $e) {
					;
		...
		*/
		...
		public function get_order_property( $object, $property) {
			switch ($property) {
				case 'id':
					return method_exists($object, 'get_id') ? $object->get_id() : null;
				case 'order_number':
					return method_exists($object, 'get_order_number') ? $object->get_order_number() : null;
				case 'payment_method_title':
					return method_exists($object, 'get_payment_method_title') ? $object->get_payment_method_title() : null;
				case 'billing_company':
					return method_exists($object, 'get_billing_company') ? $object->get_billing_company() : null;
				case 'billing_address_1':
					return method_exists($object, 'get_billing_address_1') ? $object->get_billing_address_1() : null;
				case 'billing_address_2':
					return method_exists($object, 'get_billing_address_2') ? $object->get_billing_address_2() : null;
				case 'billing_country':
					return method_exists($object, 'get_billing_country') ? $object->get_billing_country() : null;
				case 'billing_postcode':
					return method_exists($object, 'get_billing_postcode') ? $object->get_billing_postcode() : null;
				case 'billing_state':
					return method_exists($object, 'get_billing_state') ? $object->get_billing_state() : null;
				case 'billing_city':
					return method_exists($object, 'get_billing_city') ? $object->get_billing_city() : null;
				case 'billing_phone':
					return method_exists($object, 'get_billing_phone') ? $object->get_billing_phone() : null;
				case 'shipping_phone':
					return method_exists($object, 'get_shipping_phone') ? $object->get_shipping_phone() : null;
				case 'shipping_city':
					return method_exists($object, 'get_shipping_city') ? $object->get_shipping_city() : null;
				case 'shipping_state':
					return method_exists($object, 'get_shipping_state') ? $object->get_shipping_state() : null;
				case 'shipping_postcode':
					return method_exists($object, 'get_shipping_postcode') ? $object->get_shipping_postcode() : null;
				case 'shipping_country':
					return method_exists($object, 'get_shipping_country') ? $object->get_shipping_country() : null;
				case 'shipping_address_1':
					return method_exists($object, 'get_shipping_address_1') ? $object->get_shipping_address_1() : null;
				case 'shipping_address_2':
					return method_exists($object, 'get_shipping_address_2') ? $object->get_shipping_address_2() : null;
				case 'shipping_company':
					return method_exists($object, 'get_shipping_company') ? $object->get_shipping_company() : null;
				case 'email':
					return method_exists($object, 'get_billing_email') ? $object->get_billing_email() : null;
				case 'first_name':
					return method_exists($object, 'get_billing_first_name') ? $object->get_billing_first_name() : null;
				case 'last_name':
					return method_exists($object, 'get_billing_last_name') ? $object->get_billing_last_name() : null;
		...
		public function check_for_richpanel_clear() {
			if (!empty($_REQUEST) && !empty($_REQUEST['action']) && 'richpanel_clear_events' === $_REQUEST['action']) {
		...
		public function get_previous_version_settings_key() {
			...
			if (!empty($settings) && !empty($settings['api_token'])) {
				$rp_api_key = $settings['api_token'];
			}
			return $rp_api_key;
		}

		public function get_previous_version_api_key() {
			$rp_api_key = $this->get_option('api_key', false); 
			
			return $rp_api_key;	
		}

		public function get_previous_version_api_secret() {
			$rp_api_secret = $this->get_option('api_secret', false); 
			
			return $rp_api_secret;
		}
		...
		/**
		* Method to create a data layer in your WooCommerce PHP plugin and add it to the page markup.
		* Hooks into wp_head.
		*/
		public function add_data_layer() {
			...
			if ( is_product_category() ) {
				...
				$category_hash = array(
					...
					$wp_rp_data['category_details']     = $category_hash;
					$wp_rp_data['page_type']   = 'category';
			} elseif ( is_product() ) {
					$postid  = get_the_ID();
					$product = $this->resolve_product(get_queried_object_id());     
					$wp_rp_data['product']  = $this->prepare_product_hash($product);
			} else {
					$wp_rp_data['page_type'] = 'other';
			}
			wc_enqueue_js(
				'
				window.wp_rp_datalayer = ' . wp_json_encode( $wp_rp_data ) . ';'
			);
			return $wp_rp_data;
		}
	}
